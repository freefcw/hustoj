...
<ul class="breadcrumb">
    ...
    <li><a href="<?php e::url("/contest/talk?cid={$the_topic->cid}");?>">Contest <?php echo($the_topic->cid);?></a> <span class="divider"></span></li>
    <?php else:?>
...
<?php if ( OJ::is_admin() ):?>
<div class="admin-op">
    <a class="btn btn-danger make-sure" href="<?php e::url("/discuss/removetopic/{$the_topic->tid}");?>" data-no-turbolink>DELETE TOPIC</a>
    <?php if ( ! $the_topic->author()->is_disabled() ):?>
    <a class="btn btn-warning make-sure" href="<?php e::url("user/disable/{$the_topic->author_id}");?>" data-no-turbolink>DISABLE <?php echo($the_topic->author_id);?></a>
    <?php else: ?>
...
<div class="reply" id="reply-<?php echo($r->rid);?>">
    <div class="reply-header">
        <a href="<?php e::url('/u/{$r->author_id);?>');?>"><?php echo($r->author_id);?></a> reply at <?php echo(OJ::timesince($r->time));?>
